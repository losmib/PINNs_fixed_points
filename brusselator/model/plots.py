...
def brusselator_example_dynamics(PINN, path=None):
    ...
    x_fix = PINN.loss.A
    fpx_color = "red" if PINN.loss.B > PINN.loss.A**2 + 1 else "green"
    axes[0].axhline(x_fix, lw=1, ls='--', c=fpx_color)
    ...
    # include fixed point lines
    y_fix = PINN.loss.B / PINN.loss.A
    fpy_color = "red" if PINN.loss.B > PINN.loss.A**2 + 1 else "green"
    axes[1].axhline(y_fix, lw=1, ls='--', c=fpy_color)
...
def plot_brusselator_example_direction(PINN, path=None):
    # get reference solution (analytical)
    t_line, x_true, y_true = PINN.data.reference()
    ...
    x_fix = PINN.loss.A 
    y_fix = PINN.loss.B / PINN.loss.A
    col = "red" if PINN.loss.B > PINN.loss.A**2 + 1 else "green"
