...
class Loss():
    ...
    args = ['alpha', 'beta', 'delta', 'gamma', 'omega', 'x0', 'x_t0']
    ...
    def forced_duffing(self, t_col, reg_coeff):
        '''
    ...
    def physics_loss(self, t_col):
        ...
        res = x_tt - (-self.delta * x_t - self.alpha * x - self.beta * x**3 + self.gamma * tf.cos(self.omega * t_col))
        return tf.square(res), x, x_t, x_tt

    def regularizer_unstable_fp(self, t_col, x, x_t, x_tt):
        delta_num_stability = 10**-12
        lam1 = 0.5 * (-1 - tf.sqrt(tf.nn.relu(1 - 4 * (self.alpha + 3 * self.beta * x**2)) + delta_num_stability))
        lam2 = 0.5 * (-1 + tf.sqrt(tf.nn.relu(1 - 4 * (self.alpha + 3 * self.beta * x**2)) + delta_num_stability))
